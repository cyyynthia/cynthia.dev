...
description: SVG (Scalable Vector Graphics) is a widely used image format on the Web. However, due to its nature, it tends to be heavily misused and some popular tools encourage such misues. Let's sit back and look at what's wrong, and what we can do to fix it.
date: 2023-01-10
---

SVG (Scalable Vector Graphics) is a widely used image format on the Web. However, due to its nature, it tends to be
heavily misused and some popular tools encourage such misues. Let's sit back and look at what's wrong, and what we
can do to fix it.
...
The elephant in the room here is the amount of dead code served, due to the lack of icons filtering in the case Jason
...
care making my software decently efficient, I digged this rabbit hole further, and the findings were very...
disappointing.
...
demultiplies the issue! Instead of using a single shared symbols (more on them later), this effectively creates
...
it still is additonal overhead you're dealing with for every single request, which adds up.

Yes, efficiency does matter. I may in the future write in more details about why making efficient software isn't just
...
At this point, you may wonder why such a harmful practice is so popular. Well, multiple factors at at play here. First,
...
alone. Even highly qualified teams stuggle to get it right!
...
describing as harmful in this blog post. With 9.3k stars on GitHub and 21 millions downloads a month on NPM at the
...
Note that I'm rating the out-of-the-box experiences I had here. The some lacking features concern features that are
...
limitation, but I'd rather not import the 2.4k+ icons from simple-icons just beause I use 1 logo... :D
...
the SVGO config, you couldn't easily use custom code generator, etc...

### Starting from scratch
As you can tell, I wasn't satisified enough with that solution. I also recently started using [Astro](https://astro.build)
...
I've called this new project SVGSoup. For not specifc reason other than it came up in my brain and I found it had a
...
find satisfiable. I don't have much hope for a swift shift, but I hope we can focus on making sure future websites can
